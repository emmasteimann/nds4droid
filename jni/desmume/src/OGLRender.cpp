...
static void OGLRender()
{
	...
	//TODO - properly doublebuffer the display lists
	{
		...
		for(int i=0;i<gfx3d.polylist->count;i++) {
			...
			static const GLenum frm[] = {GL_TRIANGLES, GL_TRIANGLE_FAN, GL_TRIANGLE_STRIP, GL_TRIANGLE_FAN,	//TODO: GL_QUAD_STRIP
			...
			for(int j = 0; j < type; j++)
			{
				...
				int jstart = j * 4;
				memcpy(&verticies[jstart], vert->coord, sizeof(float) * 4);
				memcpy(&textureCoords[j * 2], vert->texcoord, sizeof(float) * 2);
				colors[jstart] = material_8bit_to_float[vert->color[0]];
				colors[jstart+1] = material_8bit_to_float[vert->color[1]];
				colors[jstart+2] = material_8bit_to_float[vert->color[2]];
				colors[jstart+3] = alpha;
			}
			
			glDrawArrays(frm[poly->vtxFormat], 0, type);
